...
namespace SST {
    namespace Hermes {
        namespace Shmem {
            ...
            typedef enum {
                MOVE, AND, MAX, MIN, SUM, PROD, OR, XOR
            } ReduOp;
            ...
            public:
                ...
                virtual void barrier(int start, int stride, int size, Vaddr, Callback) {
                    assert(0);
                }

                virtual void broadcast(Vaddr dest, Vaddr source, size_t nelems, int root,
                                       int PE_start,
                                       int logPE_stride, int PE_size, Vaddr, Callback) {
                    assert(0);
                }
                ...
                virtual void alltoalls(Vaddr dest, Vaddr source, int dst, int sst, size_t nelems,
                                       int elsize,
                                       int PE_start, int logPE_stride, int PE_size, Vaddr,
                                       Callback) { assert(0); }
                ...
                virtual void get(Vaddr dst, Vaddr src, size_t nelems, int pe, bool blocking,
                                 Callback) { assert(0); }

                virtual void put(Vaddr dst, Vaddr src, size_t nelems, int pe, bool blocking,
                                 Callback) { assert(0); }
                ...
                virtual void cswap(Value &result, Vaddr, Value &cond, Value &value, int pe,
                                   Callback) { assert(0); }
                ...
                virtual void fam_get(Hermes::Vaddr dest, Fam_Descriptor fd, uint64_t offset,
                                     uint64_t nbytes,
                                     bool blocking, Callback &) { assert(0); }

                virtual void fam_put(Fam_Descriptor fd, uint64_t offset, Hermes::Vaddr dest,
                                     uint64_t nbytes,
                                     bool blocking, Callback &) { assert(0); }

                virtual void fam_scatter(Hermes::Vaddr src, Fam_Descriptor fd, uint64_t nElements,
                                         uint64_t firstElement,
                                         uint64_t stride, uint64_t elmentSize, bool blocking,
                                         Callback &) { assert(0); };

                virtual void fam_scatterv(Hermes::Vaddr src, Fam_Descriptor fd, uint64_t nElements,
                                          std::vector <uint64_t> elementIndex, uint64_t elmentSize,
                                          bool blocking, Callback &) { assert(0); };

                virtual void fam_gather(Hermes::Vaddr dest, Fam_Descriptor fd, uint64_t nElements,
                                        uint64_t firstElement,
                                        uint64_t stride, uint64_t elmentSize, bool blocking,
                                        Callback &) { assert(0); };
                ...
                virtual void fam_add(Fam_Descriptor fd, uint64_t, Value &, Callback &) {
                    assert(0);
                }

                virtual void fam_cswap(Value &result, Fam_Descriptor fd, uint64_t, Value &oldValue,
                                       Value &newValue, Callback) { assert(0); }
            };
